...
from mrcs_control.cli.args.layout_args import LayoutArgs, SegmentPathAction
...
class TestLayoutArgs(unittest.TestCase):
    ...
    def test_path_up(self):
        with patch.object(sys, 'argv', ['mrcs_control_layout', 'shelf_001', '-s', 'UP', 'B1/S1', 'B2/S2']):
    ...
    def test_path_down_lowercase(self):
        with patch.object(sys, 'argv', ['mrcs_control_layout', 'shelf_001', '-s', 'dn', 'B1/S1', 'B2/S2']):
            args = LayoutArgs('test')
            self.assertEqual('shelf_001', args.layout)
    ...
    def test_path_invalid_heading(self):
        with patch.object(sys, 'argv', ['mrcs_control_layout', 'shelf_001', '-s', 'X', 'B1/S1', 'B2/S2']):
            with self.assertRaises(SystemExit):
                LayoutArgs('test')


    def test_path_missing_argument(self):
        with patch.object(sys, 'argv', ['mrcs_control_layout', 'shelf_001', '-s', 'UP', 'B1/S1']):
            with self.assertRaises(SystemExit):
                LayoutArgs('test')
    ...
    def test_path_action_none_values(self):
        action = SegmentPathAction(option_strings=['-p', '--path'], dest='path')
