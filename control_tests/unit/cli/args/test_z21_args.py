...
class TestZ21Args(unittest.TestCase):
    ...
    def test_set_mpu_drive_forward(self):
        with patch.object(sys, 'argv', ['mrcs_control_z21', '-s', '4', 'F', '128']):
            args = Z21Args('test')
    ...
    def test_set_mpu_drive_reverse_lowercase(self):
        with patch.object(sys, 'argv', ['mrcs_control_z21', '-s', '12', 'r', '0']):
            args = Z21Args('test')
    ...
    def test_set_mpu_drive_max_speed(self):
        with patch.object(sys, 'argv', ['mrcs_control_z21', '-s', '3', 'F', '255']):
            args = Z21Args('test')
    ...
    def test_set_mpu_drive_invalid_addr(self):
        with patch.object(sys, 'argv', ['mrcs_control_z21', '-s', 'abc', 'F', '100']):
    ...
    def test_set_mpu_drive_invalid_speed_above_max(self):
        with patch.object(sys, 'argv', ['mrcs_control_z21', '-s', '4', 'F', '256']):
            with self.assertRaises(SystemExit):
                Z21Args('test')


    def test_set_mpu_drive_invalid_speed_below_min(self):
        with patch.object(sys, 'argv', ['mrcs_control_z21', '-s', '4', 'F', '-1']):
            with self.assertRaises(SystemExit):
                Z21Args('test')


    def test_set_mpu_drive_invalid_speed_non_integer(self):
        with patch.object(sys, 'argv', ['mrcs_control_z21', '-s', '4', 'F', 'fast']):
            with self.assertRaises(SystemExit):
                Z21Args('test')
