...
class MPUStatusPersistence(PersistentObject, ABC):
    ...
    @classmethod
    def insert(cls, item: PersistentObject):
        client = DbClient.instance(cls.db_name())
    ...
    @classmethod
    def update_from_config(cls, config: MPUConfigurationReport):
        client = DbClient.instance(cls.db_name())
        ...
        try:
            ...
            if not row:
                raise RuntimeError(f'no MPUStatus with address 0x{config.address:02x}')
    ...
    @classmethod
    def update_from_decoder(cls, decoder: MPUDecoderReport):
        client = DbClient.instance(cls.db_name())
        ...
        try:
            ...
            if not row:
                raise RuntimeError(f'no MPUStatus with address 0x{decoder.address:02x}')
